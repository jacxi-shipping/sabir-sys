...
class SalaryPaymentWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        employee_label = QLabel(tr("Select Employee: <span style='color: red;'>*</span>"))
        employee_label.setTextFormat(Qt.RichText)
        self.employee_combo = QComboBox()
        self.employee_combo.setEditable(False)  # Prevent editing
        self.employee_combo.setToolTip(tr("Select the employee for salary payment (required)"))
        ...
        self.notes_edit.setPlaceholderText(tr("Optional notes about this payment..."))
        self.notes_edit.setToolTip(tr("Enter any additional notes about this payment (optional)"))

        self.record_button = QPushButton(tr("Record Payment"))
        self.record_button.setMinimumWidth(120)
        self.record_button.setMinimumHeight(35)
        self.record_button.setToolTip(tr("Record the salary payment (Ctrl+S)"))
        ...
        amount_label = QLabel(tr("Payment Amount: <span style='color: red;'>*</span>"))
        amount_label.setTextFormat(Qt.RichText)
        period_start_label = QLabel(tr("Pay Period Start: <span style='color: red;'>*</span>"))
        period_start_label.setTextFormat(Qt.RichText)
        period_end_label = QLabel(tr("Pay Period End: <span style='color: red;'>*</span>"))
        period_end_label.setTextFormat(Qt.RichText)
        payment_date_label = QLabel(tr("Payment Date: <span style='color: red;'>*</span>"))
    ...
    def _do_load_employees(self):
        ...
        try:
            ...
            QMessageBox.warning(self, tr("Error"), f"Failed to load employees: {str(e)}")
    ...
    def load_payment_history(self, employee_id):
        ...
        try:
            ...
            for row, payment in enumerate(payments):
                self.payments_table.setItem(row, 0, QTableWidgetItem(format_value_for_ui(payment.payment_date)))
                self.payments_table.setItem(row, 1, QTableWidgetItem(f"{payment.amount_paid:,.2f}"))
                self.payments_table.setItem(row, 2, QTableWidgetItem(format_value_for_ui(payment.period_start)))
                self.payments_table.setItem(row, 3, QTableWidgetItem(format_value_for_ui(payment.period_end)))
            ...
            QMessageBox.warning(self, tr("Error"), f"Failed to load payment history: {str(e)}")

    def record_payment(self):
        """Record salary payment"""
        current_index = self.employee_combo.currentIndex()
        if current_index < 0:
            QMessageBox.warning(self, tr("Validation Error"), "Please select an employee.")
            return
        
        employee_id = self.employee_combo.itemData(current_index)
        if not employee_id or employee_id is None:
            QMessageBox.warning(self, tr("Validation Error"), "Please select a valid employee.")
            return

        amount = self.amount_spinbox.value()
        if amount <= 0:
            QMessageBox.warning(self, tr("Warning"), "Payment amount must be positive.")
            return
    ...
    def _do_record_payment(self, employee_id, amount, period_start, period_end, payment_date, notes):
        ...
        try:
            ...
            self.loading_overlay.hide()
            QMessageBox.critical(self, tr("Error"), f"Failed to record payment: {str(e)}")
